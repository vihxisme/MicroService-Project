...
public class RabbitService<T> {
    ...
    public <T> void sendList(String exchange, String routingKey, List<T> data) {
        try {
            MessageProperties messageProperties = new MessageProperties();
            messageProperties.setDeliveryMode(MessageDeliveryMode.PERSISTENT);
    ...
    // public <T> Optional<T> deserializeToObject(byte[] jsonData, TypeReference<T> typeReference) {
    //     try {
    //         JavaType javaType = objectMapper.getTypeFactory().constructType(typeReference.getType());
    //         T object = objectMapper.readValue(jsonData, javaType);
    //         return Optional.of(object);
    //     } catch (Exception e) {
    //         logger.error("Error while deserializing to Object", e);
    //         return Optional.empty();
